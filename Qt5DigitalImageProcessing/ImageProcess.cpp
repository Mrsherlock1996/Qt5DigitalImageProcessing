...
void ImageProcess::gray(const QImage * image, QImage* result)
{
	*result = image->convertToFormat(QImage::Format_ARGB32); //QImage图片格式转换
	QColor oldColor;
	for (int y = 0; y < result->height(); y++)
	{
		for (int x = 0; x < result->width(); x++)
		{
			oldColor = QColor(image->pixel(x, y));
			int average = (oldColor.red() + oldColor.blue() + oldColor.green()) / 3;
			result->setPixel(x, y, qRgb(average, average, average));
		}
	}
...
void ImageProcess::setRGB(const QImage * image, int valueR, int valueG, int valueB, QImage* result)
{
	*result = *image;
	int r, g, b;
	QColor color;
...
void ImageProcess::adjustContrast(const QImage * src_image, int value,QImage* result)
{
	*result = *src_image;
	//value对比度增量
	/*算法概述:
			...
			2) newRGB = RGB + (RGB - Threshold) * Contrast / 255, 对比度小于0
		*/
	int pixels = result->width()*result->height(); //获取图片全部像素个数
	unsigned int* data = (unsigned int*)result->bits();//返回图像第一个像素数据指针
...
void ImageProcess::imageCenter(const QImage * qimage, const QLabel * qLabel, QImage *result)
{
	*result = *qimage;
	...
	if (dWidthRatio > dHeightRatio)
	{
		*result = qimage->scaledToWidth(labelSize.width());
	}
	else
	{
		*result = qimage->scaledToHeight(labelSize.height());
	}
...
void ImageProcess::adjustSaturation(const QImage * image, int value,QImage* result)
{
	*result = *image;
	int red, green, blue, nRed, nGreen, nBlue;
...
void ImageProcess::adjustLuminanceAndContrast(const QImage * image, int valueL, int valueC,QImage* result)
{
	//按照算法要求,有两个参数属于-1,1, 所以应对valueL和valueC做范围限制-255,255(截断处理)
	valueL = (valueL < -255) ? -255 : (valueL > 255) ? 255 : valueL;
	valueC = (valueC < -255) ? -255 : (valueC > 255) ? 255 : valueC;
	...
	*result = *image;
	int red, green, blue;
	long int pixelsNum = result->width() * result->height();
	unsigned int * data = (unsigned int*)result->bits(); //获取原图副本的像素指针
	for (int i = 0; i < pixelsNum; i++)
	{
		//像素点数值计算y = [x - 127.5 * (1 - B)] * k + 127.5 * (1 + B);
		//qRed(data[i])提取第i个像素的red通道数值
...
void ImageProcess::edge(const QImage * image, QImage* result)
{
	ConvertMatQImage cvt;
	cv::Mat matImg;
	cvt.qImageToMat(image, &matImg);
	...
	int T2 = 190;
	cv::Canny(GrayMatImg, cannyMatImg, T1, T2, 3, true);
	*result = cvt.matToQImage(&cannyMatImg);
}
...
void ImageProcess::thresholdImg(const QImage * image, int min, int max,QImage* result)
{
	ConvertMatQImage cvt;
	*result = *image;
	cv::Mat matImg;
	...
	cvt.qImageToMat(result, &matImg);

	cv::cvtColor(matImg, gray, cv::COLOR_BGR2GRAY);

	cv::threshold(gray, matImg, (double)min, (double)max, cv::THRESH_OTSU);

	 *result =  cvt.matToQImage(&matImg);
}
...
void ImageProcess::meanFilter(QImage * image, QImage* result)
{
	cv::Mat matImage;
	ConvertMatQImage cvt;
	cvt.qImageToMat(image, &matImage);
	cv::Mat matBlur;
	cv::blur(matImage, matBlur, cv::Size(3, 3), cv::Point(-1, -1));
	*result = cvt.matToQImage(&matBlur);
}
